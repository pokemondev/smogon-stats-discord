...
const UnknownPokemonEmojiName = PokemonEmoji.toEmojiName('MissingNo');
...
export class EmojiService {
  ...
  private unknownPokemonEmoji?: string;
  ...
  public getPokemonEmoji(pokemonName: string, fallbackToUnknown = true): string | undefined {
    ...
    return this.pokemonEmojiMap.get(emojiKey) ?? (fallbackToUnknown ? this.unknownPokemonEmoji : undefined);
  }
  ...
  private async loadEmojis(): Promise<void> {
    ...
    let unknownPokemonEmoji: string | undefined;
    let unknownEmoji: string | undefined;
    ...
    emojis.forEach(emoji => {
      ...
      if (emoji.name.startsWith(pokemonPrefix)) {
        ...
        pokemonMap.set(emoji.name.slice(pokemonPrefix.length), emojiString);

        if (emoji.name === UnknownPokemonEmojiName) {
          unknownPokemonEmoji = emojiString;
        }
    ...
    this.unknownPokemonEmoji = unknownPokemonEmoji;
    this.unknownEmoji = unknownEmoji;
  }
}
